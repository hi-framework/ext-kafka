...
use bytes::BytesMut;
use hi_kafka_proto::{
    CommitReq, CommitResp, ConsumerMessage, FrameType, HEADER_LEN, OffsetCommit, OffsetSpec,
    PartitionSpec, PauseResumeOp, PauseResumeReq, PauseResumeResp, PollRebalanceReq,
    ...
    TxnResp, UnsubscribeReq, codec, encode_frame,
};
...
fn ensure(socket: &str) -> Result<(), IpcError> {
    worker_health::ensure(socket).map_err(IpcError::EnsureWorker)
}
...
fn produce_sync_bin_inner(
    ...
    write_frame(&mut conn, FrameType::ProduceReq, cid, &payload, Some(timeout))?;
    ...
    if h.cid != cid {
        conn.poison();
        return Err(IpcError::CidMismatch { sent: cid, got: h.cid });
    }
...
pub fn register_cluster(
    ...
    match RegisterClusterResp::decode(&resp_bytes).map_err(|e| IpcError::DecodeResp(e.to_string()))? {
...
pub fn pause_resume(
    ...
    match PauseResumeResp::decode(&resp_bytes)
        .map_err(|e| IpcError::DecodeResp(e.to_string()))?
    {
...
pub fn send_offsets_to_transaction(
    ...
    match SendOffsetsResp::decode(&resp_bytes)
        .map_err(|e| IpcError::DecodeResp(e.to_string()))?
    {
