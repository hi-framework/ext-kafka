...
Coroutine\run(function () use ($socket, $brokers, $topic, $txnTopic, $group) {
    ...
    check($duringPause === 0, "pause 期间 poll 到 0 条新增（实际 {$duringPause}）");
    ...
    $topics = array_map(fn($p) => $p['topic'], $assigned);
    $parts  = array_map(fn($p) => $p['partition'], $assigned);
